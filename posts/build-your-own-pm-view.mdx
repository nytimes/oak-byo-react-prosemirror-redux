...
To start, let's make a function to render one of the basic units of ProseMirror
state: a ProseMirror node. Our function is going to return a DOM node
representation of our ProseMirror node. For simplicity, we'll start with text
nodes.
...
that for us, with the `contenteditable` attribute. There is no standardized
...
default! You have to add a `keymap` plugin that says _exactly_ what transactions
...
`beforeinput` event and a set of input types that express common editing
...
```javascript editor-view.js focus=6:7,14,17:19
class EditorView extends NodeView {
  constructor(dom, { state }) {
    super(state.doc, dom, null);
    this.state = state;
...
This code will have one interesting subtlety. When we refer to a text position
there are sometimes two equivalent DOM boundary points for the same ProseMirror
position because text nodes have no border. When there are two adjacent text
nodes, the boundary point after the last character of the first node and before
the first character of the second node are the same ProseMirror position. We'll
prefer not to make selections that include zero-length slices of text nodes, so
we'll make sure that we only choose a boundary point at the very start or very
end of a text node when it provides a good result. For example, we must use the
very start or very end of a text node when that node is the very first or very
last child, respectively. Otherwise, we'll prefer that selections start on the
trailing edge of such a boundary and end on the leading edge.
...
the `prosemirror-commands` package to implement a case for when `inputType` is
`insertParagraph`. Or you could try to implement a case for when `inputType` is
`deleteContentBackward`. Happy editing!
