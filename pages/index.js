export default function Index() {
  return (
    <main>
      <header>
        <h1>Welcome to Oak Learning Week!</h1>
      </header>
      <p>
        There are three subjects we&apos;ll cover this week, each with one or
        two courses. Each course will have a Glitch project that you can Remix
        so that you can follow along.
      </p>
      <p>
        We&apos;ll start with Redux! There&apos;s only one Redux course;
        we&apos;re going to dive right in by building our own Redux
        implementation from scratch.
      </p>
      <ol>
        <li>
          Build our own Redux - <a href="post/build-your-own-redux/">Course</a>{" "}
          :{" "}
          <a
            href="https://glitch.com/edit/#!/oak-pm-react-week-build-your-own-redux"
            rel="noopener noreferrer"
          >
            Glitch
          </a>
        </li>
      </ol>
      <p>Next, we&apos;re going to tackle React. We have two React courses:</p>
      <ol>
        <li>
          React Basics - <a href="post/react-basics/">Course</a> :{" "}
          <a
            href="https://glitch.com/edit/#!/oak-pm-react-week-react-basics"
            rel="noopener noreferrer"
          >
            Glitch
          </a>
        </li>
        <li>
          Build your own React -{" "}
          <a
            href="https://pomb.us/build-your-own-react/"
            rel="noopener noreferrer"
          >
            Course
          </a>{" "}
          :{" "}
          <a
            href="https://glitch.com/edit/#!/oak-pm-react-week-build-your-own-react"
            rel="noopener noreferrer"
          >
            Glitch
          </a>
        </li>
      </ol>
      <p>
        And finally, ProseMirror. Again, we have two ProseMirror courses, a
        refresher and a deep dive:
      </p>
      <ol>
        <li>
          ProseMirror Basics - <a href="post/prosemirror-basics/">Course</a> :{" "}
          <a
            href="https://glitch.com/edit/#!/oak-pm-react-week-prosemirror-basics"
            rel="noopener noreferrer"
          >
            Glitch
          </a>
        </li>
        <li>Build your own ProseMirror View - In progress!</li>
      </ol>
    </main>
  );
}
